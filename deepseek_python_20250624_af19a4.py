    ...
    for row in price_ws.iter_rows(min_row=2):
        try:
            ...
            if diff < min_diff:
                min_diff = diff
                closest_data = {
                    'B': float(row[1].value) if row[1].value else 0,
                    'C': float(row[2].value) if row[2].value else 0,
                    'D': float(row[3].value) if row[3].value else 0
                }
...
def calculate_prices_for_section(ws, price_ws, section_row, next_section_row):
    ...
    # Находим толщину стенки для секции
    thickness = None
    ...
    for row in range(section_row + 2, next_section_row + 1):
        ...
        try:
            ...
            if total_length > 0:
                price = (contour_qty * price_data['C'] + 
                        cut_length / 1000 * price_data['D'] + 
                        part_length / total_length * logistics_cost * price_data['B'])
            else:
                price = 0
